...
for region_index, region_row in df_regions.iterrows():
    ...
    # Add rolling sums
    df6['post_count_rolling'] = df6.groupby(['pax_id'])['post_count'].rolling(no_post_threshold, min_periods = 1).sum().reset_index(drop=True)
    df6['post_count_rolling_stop'] = df6.groupby(['pax_id'])['post_count'].rolling(no_post_threshold + reminder_weeks, min_periods = 1).sum().reset_index(drop=True)
    df6['post_count_rolling'] = df6.groupby(['pax_id'])['post_count'].rolling(no_post_threshold, min_periods = 1).sum().reset_index(drop=True)
    ...
    df8 = df[df['q_flag']==True].groupby(['pax_id'], as_index=False)['date'].max().rename(columns={'date':'last_q_date'})

...
def nation_sql(metadata, year_select):
...
with engine.begin() as cnxn:
    df_regions = pd.read_sql(region_sql(metadata), cnxn)
    nation_df = pd.read_sql(nation_sql(metadata, year_select), cnxn, parse_dates="date")
...
for region_row in df_regions.itertuples(index=False):
    ...
    with engine.begin() as cnxn:
        ...
        user_table = pd.read_sql(select(u), cnxn)

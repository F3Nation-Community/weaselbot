...
for region_index, region_row in df_regions.iterrows():
    ...
    pax_month_q_view2 = (
        pax_month_q_view.groupby(["month_num", "pax_id"])[["q_flag"]].count().rename(columns={"q_flag": "q_ao_count"})
    )
    ...
    for index, row in achievement_list.iterrows():
        ...
        if len(new_awards) > 0:
            for pax_index, pax_row in new_awards.iterrows():
                ...
                achievements_to_date = len(awarded_table_raw[awarded_table_raw["pax_id"] == pax_row["pax_id"]]) + len(
                    awards_add[awards_add["pax_id"] == pax_row["pax_id"]]
                )

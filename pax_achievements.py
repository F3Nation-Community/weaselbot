...
with engine.connect() as conn:
    df_regions = pd.read_sql_query(
        sql="SELECT * FROM weaselbot.regions WHERE send_achievements = 1;", con=conn
    )
...
for region_index, region_row in df_regions.iterrows():
    ...
    with engine.connect() as conn:
        ...
        awarded_table_raw = pd.read_sql_query(
            sql=awarded_view, con=conn, parse_dates=["date_awarded"]
        )
    ...
    df["ruck_flag"] = df["backblast_title"].str.contains(
        r"\b(?:pre-ruck|preruck)\b", flags=re.IGNORECASE, regex=True
    )
    df.loc[df["ao"] == "rucking", "ruck_flag"] = True
    ...
    df["blackops_flag"] = df["backblast_title"].str.contains(
        r"\b(?:blackops)\b", flags=re.IGNORECASE, regex=True
    )
    df.loc[df["ao"] == "blackops", "blackops_flag"] = True
    ...
    # Week level
    pax_week_view = pax_week_ao_view.groupby(["week_num", "pax_id"])[["bd", "q"]].agg(
    ...
    pax_week_other_view = df[df.qsource_flag][
        ["pax_id", "week_num", "qsource_flag"]
    ].drop_duplicates()
    pax_week_other_agg = (
    ...
    # Month level
    pax_month_view = pax_month_ao_view.groupby(["month_num", "pax_id"])[["bd", "q"]].agg(
        ["sum", np.count_nonzero]
    )
    ...
    pax_month_view_other = (
        ...
        .sum()
        .rename(
            columns={"qsource_flag": "qsource_sum_month", "blackops_flag": "blackops_sum_month"}
        )
    )
    # Aggregate to PAX level
    pax_month_agg = (
    ...
    pax_month_q_agg = (
        pax_month_q_view2.groupby(["pax_id"]).max().rename(columns={"q_ao_count": "q_ao_month_max"})
    )
    ...
    pax_year_ao_view = (
        df[df.bd_flag]
        .groupby(["pax_id", "ao_id"])[["bd_flag"]]
        .count()
        .rename(columns={"bd_flag": "bd_sum_ao"})
    )
    pax_year_ao_agg = (
        pax_year_ao_view.groupby(["pax_id"])[["bd_sum_ao"]]
        .max()
        .rename(columns={"bd_sum_ao": "bd_sum_ao_max"})
    )
    ...
    pax_view = reduce(
        lambda left, right: pd.merge(left, right, on=["pax_id"], how="outer"), merge_list
    ).fillna(0)
    ...
    man_achievement_df = df.loc[
        ~(df.achievement.isna()), ["pax_id", "achievement"]
    ].drop_duplicates(["pax_id", "achievement"])
    man_achievement_df["achieved"] = True
    man_achievement_df = man_achievement_df.pivot(
        index=["pax_id"], columns=["achievement"], values=["achieved"]
    )
    ...
    awarded_table = awarded_table_raw.pivot(
        index="pax_id", columns="code", values="date_awarded"
    ).reset_index()
    ...
    with engine.connect() as conn:
        awards_add.to_sql(
            name="achievements_awarded", con=conn, if_exists="append", index=False, schema=db
        )
